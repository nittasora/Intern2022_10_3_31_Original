...
namespace MySystem
{
	...
	public:
		...
		void CreateTile(const Array<TileData::Desc>& aTileDatas)
		{
			...
			{
				auto& tile = apTiles.emplace_back(new Tile{ desc, &transform });
		...
		void Draw() const noexcept
		{
			...
			ColorF color{ static_cast<double>(StagePieceData::gMAX_PIECE_NUM - pieceNumber - 1) * 0.1 };
			color.a = 1.0;
		...
		{
			return transform.GetWorlScale() * backScale;
		}

		Quad GetBody() const
		{
			return
				RectF{ Arg::center = transform.GetWorlPosition(), GetBodyScale() }.
	...
	private:
		bool ProcessAction()
		{
			...
			if (nowMoving)
			{
				...
				transform.SetWorlPosition(cursorPosition);
			}
			else if (nowScaling)
			{
				auto newScale = (Cursor::PosF() - transform.GetWorlPosition()).length() / (backScale / 2.0).length();
			...
			else if (nowRolling)
			{
				...
				if (currentDirection != Vec2::Zero())
				{
					...
					transform.SetWorlAngle(theta + transform.GetWorlAngle());
				}
			}
		...
		bool CheckAction(const Quad& body)
		{
			...
			if (isDown)
			{
				...
				prevCursorPos = Cursor::PosF();
				cursorDistance = (prevCursorPos - transform.GetWorlPosition()).lengthSq() * Max(bodyScale.x, bodyScale.y);
			}
	...
	{
		return _pImpl->transform.GetWorlAngle();
	}
}
