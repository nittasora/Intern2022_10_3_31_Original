...
namespace MySystem::StageData
{
	...
	namespace
	{
		static const String gDATA_DIRECTORY = U"Assets/Stage/data_";
	...
	{
		// オブジェクト生成に使用することが考慮されるためデータがない場合危険である
		if (not gpDesc)
		{
			CreateDefaultData();
		}
	...
	void CreateDefaultData()
	{
...

		{
...

			{
				...
				gimmick.position = Vec2{ leftTop.x + tileSize.x * 1.5, leftTop.y + tileSize.y * 8.5 };
			}

			{
				auto& gimmick = rPieceDesc.aGimmiclDesc.emplace_back();
				gimmick.gimmickType = static_cast<GimmickData::TypeIndex>(GimmickData::Type::Goal);
				gimmick.position = Vec2{ leftTop.x + tileSize.x * 8.5, leftTop.y + tileSize.y * 8.5 };
			}
		}
