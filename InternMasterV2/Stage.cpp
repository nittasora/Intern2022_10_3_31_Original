...
namespace MySystem
{
	...
	{
	private:
		using _UpdateFunction = void (Stage::Impl::*)();
		using _DrawFunction = void (Stage::Impl::*)() const;
		using _Flag = uint16;
	...
	public:
		...
		Impl(Stage* pStage, const Vec2& position, const Vec2& scale, const double angle, Transform* pParent) noexcept :
			...
			camera{ }
		{
			camera.setCenter(transform.GetWorlPosition());
		...
		void CreateStage() noexcept
		{
			...
			auto pieceData = data.lock()->aPieceDescs;
			...
			{
				apPiece[i].reset(new StagePiece{ pieceData[i], static_cast<uint8>(i), pStage, &transform});
			}
	...
	private:
		...
		void DrawPlayerAction() const
		{
			...
			for (auto hitBox : aHitBoxes)
			{
				...
				LineString{ worldHitBox.outer() }.drawClosed(Palette::Red);

				for (auto inner : worldHitBox.inners())
				{
					LineString{ inner }.drawClosed(Palette::Green);
				}
			}
		}
		...
		void UpdateGameClear()
		{
			...
			if (animationTime < CAMERA_TIME)
			{
				...
				camera.setCenter(transform.GetWorlPosition().lerp(position, rate));
			}
